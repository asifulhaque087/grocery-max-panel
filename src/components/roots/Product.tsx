...
import {
  ...
  decreaseItem,
  increaseItem,
} from '@src/graphql/reactivities/cartVariable';
import Link from 'next/link';
import { useReactiveVar } from '@apollo/client';
import { PlusIcon, MinusIcon } from '@heroicons/react/24/outline';
import { IProduct } from '@src/types/roots';
          ...
          <div className="rounded-tl rounded-tr shadow bg-gray-50n mb-1 relative cursor-pointer">
            <span
              className={`inline-block rounded-full text-white ${
                parseInt(totalSell) > 0 ? 'bg-green-500' : 'bg-yellow-500'
              } px-2 py-1 text-xs 
            font-bold mr-3 absolute top-3 left-2`}
            >
              {parseInt(totalSell) > 0 ? (
                <span>{totalSell} sell</span>
              ) : (
                'no sell'
              )}
            </span>
